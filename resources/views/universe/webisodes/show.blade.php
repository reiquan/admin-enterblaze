<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <x-universe.webisodes.show :universe="$universe" :webisode="$webisode"/>
            </div>
        </div>
    </div>
    <script>
    ...
    <script>
        ...
        function confirmDelete(universe_id, webisode_id, webisode_video_id) {

            // Display a confirmation dialog
            var userConfirmed = window.confirm('**WARNING** Deleting a webisode video will delete all content associated with it. Are you sure you want to delete video ' + webisode_video_id + '?');
            ...
            if (userConfirmed) {
                $(document).ready(function() {
                    ...
                    $.ajax({
                    url: '/universe/' + universe_id + '/webisodes/' + webisode_id + '/webisode-videos/' + webisode_video_id + '/delete?webisode_video_id=' + webisode_video_id, // Replace with your server endpoint
                    type: "POST",
                    success: function(response) {
                        // Handle success
                        console.log("Success:", response);
