<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-red-800 leading-tight">
            {{ __($card_series->card_series_name) }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <x-universe.card-series.show :card_series="$card_series" :cards="$cards"/>
            </div>
        </div>
    </div>
    <script>
        function isVisible(action, slug, id) {
            ...
            let book_id = document.getElementById("c_id" + id).value;
            let issue_id = document.getElementById("cc_id" +id).value;
