                ...
                <div class="lg:pr-8 lg:pt-4">
                  <div class="lg:max-w-lg">
                    ...
                    @if( Route::is('books.edit') )
                          @livewire('book-cover-upload', ['universe_id' => isset($_REQUEST['universe_id']) ? $_REQUEST['universe_id'] : $universe->id, 'book_id' => $book->id, 'logo' => $book->book_image_path ?? '', 'type' => 'edit'])
                    @else
                    @livewire('book-cover-upload', ['universe_id' => isset($_REQUEST['universe_id']) ? $_REQUEST['universe_id'] : $universe->id, 'book_id' => $book->id, 'logo' => $book->book_image_path ?? '', 'type' => ''])
                    @endif
                  </div>
                </div>
                <div>
                  @if(isset($book->book_image_path))
                      <h2 class="text-base my-6 font-semibold leading-7 text-indigo-600">Current Photo</h2>
                      <img src="{{ Storage::disk('s3-public')->url($book->book_image_path) }}" alt="Image" class="h-full w-full object-cover object-center lg:h-full lg:w-full"></div>
                  @else
