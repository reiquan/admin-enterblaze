<div class="bg-gray-50/70 px-4 py-6 sm:px-6 lg:px-8">
    <form method="POST" action="{{ route('cards.store',['universe_id' => isset($_REQUEST['universe_id']) ? $_REQUEST['universe_id'] : $universe->id ?? $universe_id,'card_series_id' => $card_id,  'card_id' => $card_id ]) }}" class="mx-auto max-w-6xl" enctype="multipart/form-data">
        @csrf
                    ...
                    <div>
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <h1 class="text-2xl font-bold tracking-tight text-gray-900">Upload Card Artwork</h1>
                            </div>
                        </div>
                        <p class="mt-3 max-w-2xl text-sm leading-6 text-gray-600">
                            Add the front and back artwork for this card. Both uploads stay on this page, then the main button below submits the step.
                        </p>
                    </div>
                                ...
                                <div class="p-5">
                                    ...
                                    <div data-upload-card-zone class="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4 transition hover:border-indigo-300 hover:bg-indigo-50/30">
                                        @livewire('card-upload', [
                                            'universe_id' => $universe->id ?? $universe_id ?? null,
                                            'card_series_id' => $card_series_id ?? null,
                                            'card_id' => $card_id ?? null,
                                            'current' => isset($card->card_image_one) ? Storage::disk('s3-public')->url($card->card_image_one) : null,
                                            'logo' =>  '',
                                            'field' => 'card_image_one',
                                            'type' => Route::is('card.edit') ? 'edit' : ''
                                        ], key('card-front-upload-' . ($card->id ?? 'new')))
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="border-b border-gray-100 bg-gray-50 px-5 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-purple-100 text-purple-600">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M6.429 9.75L2.25 12l4.179 2.25m0-4.5l5.571 3 5.571-3m-11.142 0L2.25 7.5 12 2.25l9.75 5.25-4.179 2.25m0 0L21.75 12l-4.179 2.25m0-4.5l-5.571 3m0 0l-5.571-3m5.571 3v8.25" />
                                            </svg>
                                        </div>
                                        <div>
                                            <h3 class="text-base font-semibold text-gray-900">Back Card Cover</h3>
                                            <p class="text-sm text-gray-500">Reverse side artwork.</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="p-5">
                                    ...
                                    <div data-upload-card-zone class="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4 transition hover:border-purple-300 hover:bg-purple-50/30">
                                        @livewire('card-upload', [
                                            ...
                                            'card_series_id' => $card_series_id ?? null,
                                            'card_id' => $card->id,
                                            'current' => isset($card->card_image_two) ? Storage::disk('s3-public')->url($card->card_image_two) : null,
                                            'logo' => $card->card_image_two ?? '',
                                            'field' => 'card_image_two',
                                            'type' => Route::is('cards.edit') ? 'edit' : ''
                                        ], key('card-series-back-upload-' . ($card->id ?? 'new')))
                                    </div>
                                </div>
                            </div>
                        </div>
