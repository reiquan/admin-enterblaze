    ...
    <div class="mx-auto max-w-7xl space-y-8">
                ...
                <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p class="mt-3 max-w-2xl text-sm leading-6 text-gray-500">
                            Create, price, publish, and manage Service bundles for your Enterblaze store and member rewards system.
                        </p>
                    </div>
        ...
        <div class="grid gap-6 md:grid-cols-3">
            <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                <p class="text-sm font-bold text-gray-500">Total services</p>
                <p class="mt-3 text-4xl font-black text-gray-950">{{ isset($services) ? $services->count() : 0 }}</p>
            </div>

            <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                <p class="text-sm font-bold text-gray-500">Published</p>
                <p class="mt-3 text-4xl font-black text-green-600">{{ isset($services) ? $services->where('service_is_active', true)->count() : 0 }}</p>
            </div>

            <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                <p class="text-sm font-bold text-gray-500">Drafts</p>
                <p class="mt-3 text-4xl font-black text-orange-500">{{ isset($services) ? $services->where('service_is_active', false)->count() : 0 }}</p>
            </div>
        </div>
        ...
        <a href="{{ route('services.create') }}"
           class="group block rounded-3xl border-2 border-dashed border-gray-300 bg-white p-10 text-center shadow-sm transition hover:border-indigo-400 hover:bg-indigo-50">
            ...
            <h2 class="mt-5 text-lg font-black text-gray-950">Create a New Service</h2>
            <p class="mt-2 text-sm text-gray-500">Build a Service package with a token amount, USD price, and publish status.</p>
        </a>
                ...
                @foreach($services as $service)
                    <div class="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
                        ...
                        <div class="grid grid-cols-3 divide-x divide-gray-200 border-t border-gray-200 bg-gray-50">
                            <div class="p-3">
                                @if($service->service_is_active)
                                    <button id="unpublish{{ $service->id }}"
                                            type="button"
                                            onclick="publishAction('unpublish', '{{ $service->id }}')"
                                            class="w-full rounded-2xl bg-yellow-100 px-4 py-3 text-sm font-black text-yellow-800 transition hover:bg-yellow-200">
                                        Unpublish
                                    </button>
                                @else
                                    <button id="publish{{ $service->id }}"
                                            type="button"
                                            onclick="publishAction('publish', '{{ $service->id }}')"
                            ...
                            <div class="p-3">
                                <form action="{{ route('services.edit', ['service_id' => $service->id]) }}" method="GET">
                                    <input type="hidden" id="service_id{{ $service->id }}" name="service_id" value="{{ $service->id }}">
                                    <button type="submit"
        ...
        @else
            <div class="rounded-3xl border border-gray-200 bg-white p-12 text-center shadow-sm">
                ...
                <h2 class="mt-5 text-2xl font-black text-gray-950">No Token services Created</h2>
                <p class="mt-2 text-sm text-gray-500">Create your first Blaze Token service to start selling token packages.</p>
